...
protocol API {
    func characters() -> AnyPublisher<[Character], APIError>
    func characterDetail(for characterID: Int) -> AnyPublisher<CharacterDetail, APIError>
...
struct MarvelAPI: API {
    ...
    func characters() -> AnyPublisher<[Character], APIError> {
        remote.load(from: url(forPath: "characters"), jsonDecoder: jsonDecoder)
            .mapError(APIError.remote)
            .eraseToAnyPublisher()
    }

    func characterDetail(for characterID: Int) -> AnyPublisher<CharacterDetail, APIError> {
        remote.load(from: url(forPath: "characters/\(characterID)"), jsonDecoder: jsonDecoder)
            .mapError(APIError.remote)
            .eraseToAnyPublisher()
    }
